...
void GuiWindow::InvalidateRegion(uint16_t u16Left_, uint16_t u16Top_, uint16_t u16Width_, uint16_t u16Height_)
{
    ...
    while (pclTempNode != 0) {
        ...
        if ((((u16Left1 >= u16Left2) && (u16Left1 <= u16Right2))
             || ((u16Right1 >= u16Left2) && (u16Right1 <= u16Right2))
             || ((u16Left1 <= u16Left2) && (u16Right1 >= u16Right2)))
            && (((u16Top1 >= u16Top2) && (u16Top1 <= u16Bottom2))
                || ((u16Bottom1 >= u16Top2) && (u16Bottom1 <= u16Bottom2))
                || ((u16Top1 <= u16Top2) && (u16Bottom1 >= u16Bottom2)))) {
            bMatch = true;
        } else if ((((u16Left2 >= u16Left1) && (u16Left2 <= u16Right1))
                    || ((u16Right2 >= u16Left1) && (u16Right2 <= u16Right1))
                    || ((u16Left2 <= u16Left1) && (u16Right2 >= u16Right1)))
                   && (((u16Top2 >= u16Top1) && (u16Top2 <= u16Bottom1))
                       || ((u16Bottom2 >= u16Top1) && (u16Bottom2 <= u16Bottom1))
...
void GuiWindow::ProcessEvent(GuiEvent_t* pstEvent_)
{
    ...
    if ((TARGET_ID_BROADCAST == pstEvent_->u8TargetID) || (TARGET_ID_BROADCAST_Z == pstEvent_->u8TargetID)) {
...
bool GuiEventSurface::ProcessEvent()
{
    ...
    CopyEvent(&stLocalEvent, static_cast<GuiEvent_t*>(pclMessage->GetData()));
